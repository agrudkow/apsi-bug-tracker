...
report_keyword_association_table = Table('report_keyword_association', Base.metadata,
                                         Column('key_word_id', ForeignKey(
                                             'reports.id'), index=True),
                                         Column('report_id', ForeignKey(
                                             'key_words.id'))
                                         )
...
class KeyWord(Base):
    ...
    reports = relationship(
        "Report", secondary=report_keyword_association_table, back_populates="key_words")
...
class Report(Base):
    ...
    urgency_level = Column(String(1), ForeignKey(
        'urgency_levels.level'), index=True)
    report_class = Column(String(1), ForeignKey(
        'report_classes.id'), index=True)
    bug_id = Column(String(10), ForeignKey('bugs.id'), index=True)
    module_id = Column(String(3), ForeignKey('modules.id'), index=True)
    key_words = relationship(
        "KeyWord", secondary=report_keyword_association_table, back_populates="reports")
...
class Message(Base):
    ...
    username = Column(String(50), ForeignKey(
        'users.username'), primary_key=True, index=True)
...
class Role(Base):
    ...
    # Foreign keys
    report_id = Column(String(6), ForeignKey(
        'reports.id'), primary_key=True, index=True)
    username = Column(String(50), ForeignKey(
